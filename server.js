...
// 关掉静态文件缓存：避免手机浏览器一直用旧版 app.js/index.html，导致更新了代码却看起来没生效
...
app.post('/api/register', asyncHandler(async (req, res) => {
  ...
  let result;
  try {
    // 直接尝试插入，靠 UNIQUE 约束判断用户名是否重复，比先查一遍再插入少一次数据库往返
    result = await db.run(
...
app.post('/api/login', asyncHandler(async (req, res) => {
  ...
  req.session.userId = user.id;
  req.session.displayName = user.display_name;
  res.json({ user: { id: user.id, username: user.username, display_name: user.display_name, avatar: user.avatar } });
}));
...
app.post('/api/ingredients', requireAuth, asyncHandler(async (req, res) => {
  ...
  const result = await db.run(
    ...
    [trimmedName, cat, emoji, req.session.userId]
  );
  // 直接用已知的数据拼返回值，不用再查一次数据库
  res.json({
    ingredient: {
      id: Number(result.lastInsertRowid),
      name: trimmedName,
...
app.post('/api/board', requireAuth, asyncHandler(async (req, res) => {
  ...
  // 一次查询顺便拿到食材的名字/图标/分类，省掉后面单独查一次
  const ing = await db.get('SELECT id, name, emoji, category FROM ingredients WHERE id = ?', [ingredientId]);
  ...
  res.json({
    item: {
      ...
      added_by_name: req.session.displayName || null,
    },
  });
}));
