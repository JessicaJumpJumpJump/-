...
// ---------- 加入某一餐（拖拽和点击共用） ----------
async function addIngredientToSlot(ingredientId, slot) {
  await api('/api/board', {
    method: 'POST',
    body: JSON.stringify({ date: currentDate, slot, ingredientId }),
  });
  await loadBoard();
}

// ---------- 手机用的选餐弹窗（点一下食材卡片后弹出） ----------
const sheetOverlay = document.getElementById('slotSheetOverlay');
const sheetTitle = document.getElementById('slotSheetTitle');
const sheetButtons = document.getElementById('slotSheetButtons');

function openSlotPicker(ing) {
  sheetTitle.textContent = `${ing.emoji} ${ing.name} 加入哪一餐？`;
  ...
  for (const slot of Object.keys(SLOT_LABELS)) {
    ...
    btn.addEventListener('click', async () => {
      closeSlotPicker();
      await addIngredientToSlot(ing.id, slot);
    });
...
sheetOverlay.addEventListener('click', (e) => {
  if (e.target === sheetOverlay) closeSlotPicker();
});
document.getElementById('slotSheetCancel').addEventListener('click', closeSlotPicker);
...
document.querySelectorAll('.slot-drop').forEach((zone) => {
  ...
  zone.addEventListener('drop', async (e) => {
    ...
    await addIngredientToSlot(ingredientId, zone.dataset.slot);
  });
});
...
async function loadBoard() {
  ...
  document.querySelectorAll('.slot-drop').forEach((zone) => {
    ...
    if (slotItems.length === 0) {
      zone.innerHTML = `<div class="empty-msg">点/拖一个食材到这里～</div>`;
      return;
    }
